...
! ----------------------------------------------------------------------------
  public mesh,mesh_eqv,get_meshpoint
! ----------------------------------------------------------------------------
...
! *** FUNCTION mesh_eqv <<<
! ----------------------------------------------------------------------------
logical function mesh_eqv(mesh1,mesh2)

  ! Arguments
  type(mesh),intent(in) :: mesh1,mesh2

  mesh_eqv = (mesh1%N.EQ.mesh2%N .AND. mesh1%type.EQ.mesh2%type)

  return
end function mesh_eqv
! ----------------------------------------------------------------------------
...
! ----------------------------------------------------------------------------
function get_meshpoint_single(msh,idx) result(x_idx)
  ...
  if ( .NOT.init_x .OR. .NOT.mesh_eqv(msh,msh_save) ) then
      if (ALLOCATED(x_save)) deallocate(x_save)
      allocate(x_save(0:msh%N+1))

      select case (msh%type)
      case ('UN');  call Setup_Mesh_uniform(msh%N)
      case ('SH');  call Setup_Mesh_Shishkin(msh%N)
      case ('BA');  call Setup_Mesh_Bakhvalov(msh%N)
      case default; call STOP_ON_ERROR( 'get_meshpoint (in MESHES)',         &
...
! ----------------------------------------------------------------------------
function get_meshpoint_all(msh) result(x)

  ! Arguments
  type(mesh),intent(in) :: msh

  ! Result
  real(RNP),dimension(0:msh%N+1) :: x

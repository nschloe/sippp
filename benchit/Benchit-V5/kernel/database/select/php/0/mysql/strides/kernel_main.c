...
void bi_getinfo( bi_info * pinfo )
{
   ...
   pinfo->kerneldescription = bi_strdup( "blockwise reading from database" );
   pinfo->xaxistext = bi_strdup( "Stripe Size read from Database" );
   pinfo->base_xaxis = 2;
...
void evaluate_environment(mydata_t * pmydata)
{
  ...
  p = bi_getenv( "BENCHIT_PHP_INTERPRETER", 0 );
  if ( p == NULL ) errors++;
  ...
  /* read the database specific values */
  p = bi_getenv( "BENCHIT_KERNEL_DB_SERVER", 0 );
  if ( p == NULL ) errors++;
  else pmydata->dbserver = bi_strdup( p );

  p = bi_getenv( "BENCHIT_KERNEL_DB_NAME", 0 );
  if ( p == NULL ) errors++;
  else pmydata->dbname = bi_strdup( p );
	
  p = bi_getenv( "BENCHIT_KERNEL_DB_USR", 0 );
    if ( p == NULL ) errors++;
  else pmydata->dbuser = bi_strdup( p );

	p = bi_getenv( "BENCHIT_KERNEL_DB_PASS", 0 );
  if ( p == NULL ) errors++;
  else pmydata->dbpass = bi_strdup( p );
...
char* bi_build_work_script( char* script_file, char *meas_mode, void* pdata )
{
  ...
  work_script = bi_strcat( work_script, script_file );
	work_script = bi_strcat( work_script, " -DBServer " );
	work_script = bi_strcat( work_script, pmydata->dbserver );
	work_script = bi_strcat( work_script, " -DBName " );
	work_script = bi_strcat( work_script, pmydata->dbname );
	work_script = bi_strcat( work_script, " -DBUser " );
	work_script = bi_strcat( work_script, pmydata->dbuser );
	work_script = bi_strcat( work_script, " -DBPass " );
	work_script = bi_strcat( work_script, pmydata->dbpass );
...
void* bi_init( int problemsizemax )
{
  ...
  evaluate_environment(pmydata);
	...
	work_script = bi_build_work_script( "work_script.php", "init", pmydata );
...
int bi_entry( void* mdpv, int iproblemsize, double* dresults )
{
  ...
  if ( dresults == NULL ) return 1;
	...
	work_script = bi_build_work_script( "work_script.php", "read", pmydata );
  work_script = bi_strcat( work_script, " -BIProblemSize " );

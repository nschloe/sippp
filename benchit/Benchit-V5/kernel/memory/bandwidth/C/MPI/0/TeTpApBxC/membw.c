...
void bi_getinfo(bi_info* infostruct){
  ...
  infostruct->codesequence=bi_strdup("for i=1,N#  temp+=a[i]+b[i]*c[i]#");
...
int bi_entry(void *mcb,int problemsize,double *results) {
  ...
  for(numproc=1;numproc<=size;numproc++){    
    ...
    result=mem_read( a, b, c, vectorsize);
#ifdef USE_MPI
    MPI_Barrier(MPI_COMM_WORLD);
#endif
    if(rank<numproc){
      start=bi_gettime();
      for (i=0;i<repeats;i++) result+=mem_read( a, b, c, vectorsize);
    }
#ifdef USE_MPI
    MPI_Barrier(MPI_COMM_WORLD);
#endif
    ...
    if (rank==0){
      results[numproc]=(double)((double)(problemsize)*numproc/((stop-start-calloh)/repeats-dTimerOverhead));
    }
  }
  return (0);
}

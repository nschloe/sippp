...
void bi_getinfo( bi_info * pinfo )
{
   ...
   pinfo->kerneldescription = bi_strdup( "Bubblesort Integer" );
   ...
   /* setting up y axis texts and properties */
      ...
      pinfo->legendtexts[0] = bi_strdup( "integer elements" );
...
void* bi_init( int problemsizemax )
{
   ...
   pmydata->intarray = (myinttype *)(malloc( sizeof( myinttype) * pmydata->steps) );
...
int bi_entry( void * mdpv, int iproblemsize, double * dresults )
{
  ...
  myinttype * psortarray = NULL;

  /* calculate real problemsize */
  imyproblemsize = pmydata->min + ( (imyproblemsize - 1)  * pmydata->increment );

  psortarray = (myinttype *)(malloc( sizeof( myinttype) * imyproblemsize) );
  for ( ii = 0; ii < imyproblemsize; ii++ )
  {
       psortarray[ii] = pmydata->intarray[ii];
  }
  ...
  dstart = bi_gettime(); 
  dres = 0; 
  bubblesorti( psortarray, imyproblemsize );
  dend = bi_gettime();
...
//  fprintf( stderr, "Problemsize=%d, Value=%f\n", imyproblemsize, dres);
  ...
  */
  ...
  if ( !( verifyi(psortarray,imyproblemsize)) ) printf( "Verification intsort failed! \n" );

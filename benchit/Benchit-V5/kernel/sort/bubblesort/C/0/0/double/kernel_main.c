...
void bi_getinfo( bi_info * pinfo )
{
   ...
   pinfo->kerneldescription = bi_strdup( "Bubblesort Double" );
   ...
   /* setting up y axis texts and properties */
      ...
      pinfo->legendtexts[0] = bi_strdup( "double elements" );
...
void* bi_init( int problemsizemax )
{
   ...
   pmydata->doublearray = (double *)(malloc( sizeof( double) * pmydata->steps ) );
   ...
   for ( ii = 0; ii < pmydata->steps; ii++ )
      {
      pmydata->doublearray[ii] = ( double ) ( random() + random() /
                                   random() * pow( random(), 3 ) );
      }
...
int bi_entry( void * mdpv, int iproblemsize, double * dresults )
{
  ...
  psortarray = (double *)(malloc( sizeof( double) * imyproblemsize) );
  for ( ii = 0; ii < imyproblemsize; ii++ )
  {
       psortarray[ii] = pmydata->doublearray[ii];
  }
  ...
  dstart = bi_gettime(); 
  dres = 0;
  bubblesortd( psortarray, imyproblemsize );
  dend = bi_gettime();

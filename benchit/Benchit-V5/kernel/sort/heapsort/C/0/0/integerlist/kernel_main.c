...
void bi_getinfo( bi_info * pinfo )
{
   ...
   pinfo->kerneldescription = bi_strdup( "Heapsort Integerlist" );
   pinfo->xaxistext = bi_strdup( "number of elements" );
   //pinfo->maxproblemsize = penv->steps;
   parse( &ii, penv->plistenv );
   pinfo->maxproblemsize = ii;
...
//   list * p;
   ...
   pmydata->plist = parse( &ii, pmydata->plistenv );
...
int bi_entry( void * mdpv, int iproblemsize, double * dresults )
{
  ...
  {
       psortarray[ii+1] = pmydata->plist[ii].lnumber;
  }

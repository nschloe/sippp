...
void bi_getinfo( bi_info * pinfo )
{
   ...
   pinfo->kerneldescription = bi_strdup( "Heapsort Integer" );
...
int bi_entry( void * mdpv, int iproblemsize, double * dresults )
{
  ...
  psortarray = (myinttype *)(malloc( sizeof( myinttype) * imyproblemsize + 1) );
  for ( ii = 0; ii < imyproblemsize; ii++ )
  {
       psortarray[ii+1] = pmydata->intarray[ii];
  }
  ...
  dstart = bi_gettime(); 
  dres = 0; 
  heapsorti( psortarray, imyproblemsize );
  dend = bi_gettime();

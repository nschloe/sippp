...
void bi_getinfo(bi_info* pinfo)
{
   ...
   pinfo->kerneldescription = bi_strdup("1D Fast Fourier Transform, Non-Powers of 2, single precision, complex data, SCSL (C)");
...
int bi_entry(void * mdpv, int iproblemsize, double * dresults)
{
   ...
   /* malloc */
   pmydata->table = (float*)malloc(sizeof(float) * (2 * imyproblemsize + 256));
   pmydata->work = (float*)malloc(sizeof(float) * (2 * imyproblemsize));
   pmydata->inout = (float*)malloc(sizeof(float) * imyproblemsize * 2);

   /* create FFT plan */
   status = ccfft(0, imyproblemsize, 1.0, NULL, NULL, pmydata->table, NULL, isys);
   ...
   dstart = bi_gettime();
   /* fft calculation */
   status = ccfft(-1, imyproblemsize, 1.0, (scsl_complex*)pmydata->inout, (scsl_complex*)pmydata->inout, pmydata->table, pmydata->work, isys);
   dend = bi_gettime();
   ...
   dstart = bi_gettime();
   /* fft calculation */
   status = ccfft(-1, imyproblemsize, 1.0, (scsl_complex*)pmydata->in, (scsl_complex*)pmydata->out, pmydata->table, pmydata->work, isys);
   dend = bi_gettime();

 ...
 *          double precision, complex data, SCSL (C)
...
void bi_getinfo(bi_info* pinfo)
{
   ...
   pinfo->kerneldescription = bi_strdup("1D Fast Fourier Transform, Non-Powers of 2, double precision, complex data, SCSL (C)");
...
int bi_entry(void * mdpv, int iproblemsize, double * dresults)
{
   ...
   /* malloc */
   pmydata->table = (double*)malloc(sizeof(double) * (2 * imyproblemsize + 256));
   pmydata->work = (double*)malloc(sizeof(double) * (2 * imyproblemsize));
   pmydata->inout = (double*)malloc(sizeof(double) * imyproblemsize * 2);

   /* create FFT plan */
   status = zzfft(0, imyproblemsize, 1.0, NULL, NULL, pmydata->table, NULL, isys);
   ...
   dstart = bi_gettime();
   /* fft calculation */
   status = zzfft(-1, imyproblemsize, 1.0, (scsl_zomplex*)pmydata->inout, (scsl_zomplex*)pmydata->inout, pmydata->table, pmydata->work, isys);
   dend = bi_gettime();
   ...
   dstart = bi_gettime();
   /* fft calculation */
   status = zzfft(-1, imyproblemsize, 1.0, (scsl_zomplex*)pmydata->in, (scsl_zomplex*)pmydata->out, pmydata->table, pmydata->work, isys);
   dend = bi_gettime();

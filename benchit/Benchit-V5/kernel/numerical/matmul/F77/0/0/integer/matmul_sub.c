...
void ( *entry1 ) ( myinttype *a, myinttype *b, myinttype *c, myinttype *size );
...
void init_( fds *myfds, int *size )
{
  ...
  IDL( 3, printf( "field size: %ld bytes", ( long ) * size * ( *size ) * sizeof( myinttype ) ) );
  for ( x = 0; x < *size; x++ )
    for ( y = 0; y < *size; y++ )
    {
      index = x * ( *size ) + y;
...
{
  fds * myfds = ptr;
  int *f1 = myfds->feld1, *f2 = myfds->feld2, *f3 = myfds->feld3;
  if ( *size == 0 )
    return ;
  else
...
void bi_getinfo( bi_info* infostruct )
{
  ...
  infostruct->kerneldescription = bi_strdup( "Matrix Multiply (F77) Integer" );
...
int bi_entry( void* mdpv, int problemsize, double* results )
{
  ...
  for ( v = 0; v < n_of_works; v++ )
  {
    ...
    if ( time < dTimerGranularity )
      ...
      results[ v + 1 ] = ( ( double ) count ) / (time*numberOfRuns);
  }
  return 0;
}
...
void *bi_init( int problemsizemax )
{
  ...
  myfds->feld1 = malloc( ( problemsizemax ) * ( problemsizemax ) * sizeof( myinttype ) );
  IDL( 3, printf( "Alloc 1 done\n" ) );
  myfds->feld2 = malloc( ( problemsizemax ) * ( problemsizemax ) * sizeof( myinttype ) );
  IDL( 3, printf( "Alloc 2 done\n" ) );
  myfds->feld3 = malloc( ( problemsizemax ) * ( problemsizemax ) * sizeof( myinttype ) );
       ...
       ( myfds->feld3 == NULL ) )
  {
    printf( "malloc (%ld bytes) failed in bi_init()\n",
            ( long ) ( 3.0 * problemsizemax * problemsizemax * sizeof( myinttype ) ) );
    bi_cleanup( myfds );
    exit( 127 );
  }
 ...
 * 
 * $Log: matmul_sub.c,v $
 * Revision 1.3  2006/05/09 09:33:29  rschoene
 * removed bug: count_ delivered an unsigned long, which isn't long enough,
 * changed to double
 *
 * Revision 1.2  2006/04/24 11:32:56  rschoene
 * added descriptions, removed warning
 *

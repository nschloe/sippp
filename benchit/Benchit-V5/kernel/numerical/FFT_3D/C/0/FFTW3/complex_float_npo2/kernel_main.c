 ...
 *          single precision, complex data, FFTW3 (C)
...
void bi_getinfo(bi_info* pinfo)
{
   ...
   pinfo->codesequence = bi_strdup("fftwf_execute()");
   pinfo->kerneldescription = bi_strdup("3D Fast Fourier Transform, Non-Powers of 2, single precision, complex data, FFTW3 (C)");
...
int bi_entry(void * mdpv, int iproblemsize, double * dresults)
{
   ...
   /* malloc */
   pmydata->inout = (fftwf_complex*)fftwf_malloc(sizeof(fftwf_complex) * imyproblemsize * imyproblemsize * imyproblemsize);

   /* create FFT plan */
   pmydata->p = fftwf_plan_dft_3d(imyproblemsize, imyproblemsize, imyproblemsize, pmydata->inout, pmydata->inout, FFTW_FORWARD, FFTW_ESTIMATE);
   ...
   fftwf_destroy_plan(pmydata->p);

   /* free data */
   fftwf_free(pmydata->inout);


   /*** out of place run ***/

   /* malloc */
   pmydata->in = (fftwf_complex*)fftwf_malloc(sizeof(fftwf_complex) * imyproblemsize * imyproblemsize * imyproblemsize);
   pmydata->out = (fftwf_complex*)fftwf_malloc(sizeof(fftwf_complex) * imyproblemsize * imyproblemsize * imyproblemsize);

   /* create FFT plan */
   pmydata->p = fftwf_plan_dft_3d(imyproblemsize, imyproblemsize, imyproblemsize, pmydata->in, pmydata->out, FFTW_FORWARD, FFTW_ESTIMATE);
   ...
   /* free data */
   fftwf_free(pmydata->in);
   fftwf_free(pmydata->out);
...
void bi_cleanup(void* mdpv)
{
   mydata_t* pmydata = (mydata_t*)mdpv;
   if (pmydata) {
   fftwf_free(pmydata->problemsizes);
      free(pmydata);
   }
   return;
}

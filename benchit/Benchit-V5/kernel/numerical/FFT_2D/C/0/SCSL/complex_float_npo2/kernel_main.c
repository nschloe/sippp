 ...
 *          single precision, complex data, SCSL (C)
...
void bi_getinfo(bi_info* pinfo)
{
   ...
   pinfo->kerneldescription = bi_strdup("2D Fast Fourier Transform, Non-Powers of 2, single precision, complex data, SCSL (C)");
...
int bi_entry(void * mdpv, int iproblemsize, double * dresults)
{
   ...
   /* malloc */
   pmydata->table = (float*)malloc(sizeof(float) * 2 * (2 * imyproblemsize + 256));
   ...
   /* create FFT plan */
   status = ccfft2d(0, imyproblemsize, imyproblemsize, 1.0, NULL, imyproblemsize, NULL, imyproblemsize, pmydata->table, NULL, isys);
   ...
   dstart = bi_gettime();
   /* fft calculation */
   status = ccfft2d(-1, imyproblemsize, imyproblemsize, 1.0, (scsl_complex*)pmydata->inout, imyproblemsize, (scsl_complex*)pmydata->inout, imyproblemsize, pmydata->table, pmydata->work, isys);
   dend = bi_gettime();
   ...
   dstart = bi_gettime();
   /* fft calculation */
   status = ccfft2d(-1, imyproblemsize, imyproblemsize, 1.0, (scsl_complex*)pmydata->in, imyproblemsize, (scsl_complex*)pmydata->out, imyproblemsize, pmydata->table, pmydata->work, isys);
   dend = bi_gettime();

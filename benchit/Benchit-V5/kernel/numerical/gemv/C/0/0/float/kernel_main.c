...
void* bi_init( int problemsizemax )
{
  ...
  problemsizemax=MAX;
  mdp->x=malloc( (problemsizemax*problemsizemax) * sizeof(float));
  IDL( 3, printf("Alloc 1 done\n"));
  mdp->y=malloc( (problemsizemax) * sizeof(float));
  IDL( 3, printf("Alloc 2 done\n"));
  mdp->a=malloc( (problemsizemax) * sizeof(float));
...
void evaluate_environment()
{
  ...
  MAX = 500;
  INCREMENT = 1;
  p = bi_getenv( "BENCHIT_SGEMV_C_FLOAT_MIN", 0 );
  if ( p == 0 ) errors++;
  else MIN = atoi( p );
  p = bi_getenv( "BENCHIT_SGEMV_C_FLOAT_MAX", 0 );
  if ( p == 0 ) errors++;
  else MAX = atoi( p );
  p = bi_getenv( "BENCHIT_SGEMV_C_FLOAT_INCREMENT", 0 );
  if ( p == 0 ) errors++;
  ...
  if ( errors > 0 )
  {
    ...
    fprintf( stderr, "BENCHIT_SGEMV_C_FLOAT_MIN\n" );
    fprintf( stderr, "BENCHIT_SGEMV_C_FLOAT_MAX\n" );
    fprintf( stderr, "BENCHIT_SGEMV_C_FLOAT_INCREMENT\n" );
...
$Log: kernel_main.c,v $
Revision 1.4  2007/07/06 05:43:02  rschoene
moved declarations
...
Revision 1.1  2006/04/05 07:24:03  rschoene
gemv, single precision

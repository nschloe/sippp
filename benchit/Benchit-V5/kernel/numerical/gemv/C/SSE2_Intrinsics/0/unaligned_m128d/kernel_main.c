      ...
      switch ( j ) {
         case 1:
                 do
                 {
                   ...
                   {
                     sseunalignJI_(problemsize,problemsize, 1.1,0.9,mdp->a, mdp->x, mdp->y);
                   }
                   stop=bi_gettime();
         ...
         case 0:
                 do
                 {
                   ...
                   start=bi_gettime();
                   for (i=0;i<numberOfRuns;i++)
                     sseunalignIJ_(problemsize,problemsize, 1.1,0.9,mdp->a, mdp->x, mdp->y);
                   stop=bi_gettime();
                   stop=stop-start-dTimerOverhead;
                   numberOfRuns=numberOfRuns*8;
                 } while ( stop<0.01 );
...
void evaluate_environment()
{
  ...
  INCREMENT = 1;
  MAX = 600;
  p = bi_getenv( "BENCHIT_DGEMV_C_SSE2_UNALIGN_MIN", 0 );
  if ( p == 0 ) errors++;
  else MIN = atoi( p );
  p = bi_getenv( "BENCHIT_DGEMV_C_SSE2_UNALIGN_INCREMENT", 0 );
  if ( p == 0 ) errors++;
  else INCREMENT = atoi( p );
  p = bi_getenv( "BENCHIT_DGEMV_C_SSE2_UNALIGN_MAX", 0 );
  if ( p == 0 ) errors++;
  else MAX = atoi( p );
  if ( errors > 0 )
  {
    ...
    fprintf( stderr, "BENCHIT_DGEMV_C_SSE2_UNALIGN_MIN\n" );
    fprintf( stderr, "BENCHIT_DGEMV_C_SSE2_UNALIGN_INCREMENT\n" );
    fprintf( stderr, "BENCHIT_DGEMV_C_SSE2_UNALIGN_MAX\n" );

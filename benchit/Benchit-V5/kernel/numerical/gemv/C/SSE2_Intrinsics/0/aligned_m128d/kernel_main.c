...
void bi_getinfo( bi_info * infostruct )
{
  ...
  infostruct->kerneldescription = bi_strdup( "Matrix Vector Multiply (C + SSE2)" );
...
void* bi_init( int problemsizemax )
{
  ...
  problemsizemax=MAX;
  mdp->x=_mm_malloc( (problemsizemax*problemsizemax) * sizeof(double),16);
  IDL( 3, printf("Alloc 1 done\n"));
  mdp->y=_mm_malloc( (problemsizemax) * sizeof(double),16);
  IDL( 3, printf("Alloc 2 done\n"));
  mdp->a=_mm_malloc( (problemsizemax) * sizeof(double),16);
...
void evaluate_environment()
{
  ...
  INCREMENT = 2;
  MAX = 400;
  p = bi_getenv( "BENCHIT_DGEMV_C_SSE2_ALIGN_MIN", 0 );
  if ( p == 0 ) errors++;
  else MIN = atoi( p );
  p = bi_getenv( "BENCHIT_DGEMV_C_SSE2_ALIGN_INCREMENT", 0 );
  if ( p == 0 ) errors++;
  else INCREMENT = atoi( p );
  p = bi_getenv( "BENCHIT_DGEMV_C_SSE2_ALIGN_MAX", 0 );
  if ( p == 0 ) errors++;
  else MAX = atoi( p );
  if ( errors > 0 )
  {
    ...
    fprintf( stderr, "BENCHIT_DGEMV_C_SSE2_ALIGN_MIN\n" );
    fprintf( stderr, "BENCHIT_DGEMV_C_SSE2_ALIGN_MAX\n" );
    fprintf( stderr, "BENCHIT_DGEMV_C_SSE2_ALIGN_INCREMENT\n" );
...
gemv, double precision sse2 aligned

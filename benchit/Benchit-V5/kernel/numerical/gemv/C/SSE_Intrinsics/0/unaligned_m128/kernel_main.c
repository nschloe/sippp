...
double overheadIJ=0.0;
double overheadJI=0.0;
double overheadTimer=0.0;
...
void evaluate_environment()
{
  ...
  INCREMENT = 1;
  MAX = 600;
  p = bi_getenv( "BENCHIT_SGEMV_C_SSE_UNALIGN_MIN", 0 );
  if ( p == 0 ) errors++;
  else MIN = atoi( p );
  p = bi_getenv( "BENCHIT_SGEMV_C_SSE_UNALIGN_INCREMENT", 0 );
  if ( p == 0 ) errors++;
  else INCREMENT = atoi( p );
  p = bi_getenv( "BENCHIT_SGEMV_C_SSE_UNALIGN_MAX", 0 );
  if ( p == 0 ) errors++;
  else MAX = atoi( p );
  if ( errors > 0 )
  {
    ...
    fprintf( stderr, "BENCHIT_SGEMV_C_SSE_UNALIGN_MIN\n" );
    fprintf( stderr, "BENCHIT_SGEMV_C_SSE_UNALIGN_INCREMENT\n" );
    fprintf( stderr, "BENCHIT_SGEMV_C_SSE_UNALIGN_MAX\n" );

...
static int NUM_KERNEL_VERSIONS = 2;
...
void bi_getinfo(bi_info *info)
{
        ...
        info->yaxistexts[0] = bi_strdup("No. of floating points ops");
        info->yaxistexts[1] = bi_strdup("No. of floating points ops");
	...
	/* Legend texts */
        ...
        info->legendtexts[0] = bi_strdup("LowerBlock ver.1");
        info->legendtexts[1] = bi_strdup("LowerBlock ver.2");
...
int bi_entry(void *data, int N, double *results)
{
        ...
        if (T_x == 0.0)
        {
                for (i=0;i<N_x;i++)
                {
                        individual_init_(&problem_size);
                        T_0 = bi_gettime();
                        kernel_main_(&N_0,&N_1,&num_flop);
                        T_1 = bi_gettime();
                        T_x += T_1 - T_0;
                        individual_cleanup_();
                }

...
/**
 * The BIGEditor provides you, to edit the local def files manually.
 * It is synchronized with the BIGGUI, so changing a BIGEntry in
 * the BIGEditor, the BIGGUI were also changed.<br>
 * The syntax highlighting is also possible to get, what kind of type
 * an entry is.<br><br>
 *
 */
public class BIGEditor
    extends JComponent implements Printable
{
   /**
   ...
   public BIGEditor( File f )
   {
      ...
      {
         this.setTokenMarker( new CTokenMarker() ) ;
      }
      ...
      {
         this.setTokenMarker( new CCTokenMarker() ) ;
      }
      else if ( extension.equals( "sh" ) || extension.equals( "SH" ) ||
                ( f.getName().indexOf( "." ) == -1 ) )
      {
         this.setTokenMarker( new ShellScriptTokenMarker() ) ;
      }
      else if ( extension.equals( "php" ) || extension.equals( "php3" ) ||
                extension.equals( "phps" ) )
      {
         this.setTokenMarker( new PHPTokenMarker() ) ;
      }
      else if ( extension.equals( "perl" ) || extension.equals( "pl" ) ||
                extension.equals( "pm" ) )
      {
         this.setTokenMarker( new PerlTokenMarker() ) ;
      }
      else if ( extension.equals( "py" ) )
      {
         this.setTokenMarker( new PythonTokenMarker() ) ;
      }

      /* else if (extension.equals("f")||extension.equals("f77")||extension.equals(".f90"))
                  {
                      this.setTokenMarker(new FortranTokenMarker());
                  }*/
      ...
      this.setText( system.BIGFileHelper.getFileContent( f ) ) ;
...
//            this.initText=this.getText();
      ...
      painter = new TextAreaPainter( this , defaults )
          {
             public void paint(Graphics gfx)
             {
                ...
                g2d.fillRect(0,0,this.getFontMetrics().charWidth('W')*5,
                             getFontMetrics().getHeight()*getLineCount());
         ...
         public void mouseWheelMoved( java.awt.event.MouseWheelEvent evt )
         {
            if ( evt.isControlDown() )
            {
               horizontal.setValue( horizontal.getValue() +
                                    evt.getWheelRotation() * 50 ) ;
            }
            else vertical.setValue( vertical.getValue() +
   ...
   private void initStuff()
   {
      ...
      try
      {
         Font f = new Font( "Monospaced" , Font.PLAIN ,
                            system.BIGInterface.getInstance().
                            getBIGConfigFileParser().intCheckOut( "editorTextSize" ) ) ;
         painter.setFont( f ) ;
         this.setFont(f);
      }
      catch ( Exception ex )
      ...
      java.util.Dictionary dic = ( ( PlainDocument )this.document ).
          getDocumentProperties() ;
      java.util.Enumeration en = dic.keys() ;
      ...
      org.syntax.jedit.DefaultInputHandler han = new org.syntax.jedit.
          DefaultInputHandler()
      {
         public void keyPressed( KeyEvent evt )
         {
            ...
            if ( ( evt.getKeyCode() == 10 ) )
            {
               ...
               //setText( t.substring( 0 , off ) + start + t.substring( off ) ) ;
               setCaretPosition( off + start.length() ) ;
               setFirstLine( firstLine ) ;
               document.tokenizeLines() ;
            }
            JPanel console = system.BIGInterface.getInstance().
                getConsole().getDisplayPanel() ;
            BIGGUI gui = ( BIGGUI ) console.getRootPane().getParent() ;
            // modifiers 2 means ctrl
            if ( ( evt.getModifiers() == 2 ) )
            {
               if (evt.getKeyText(evt.getKeyCode()).equals("P"))
               {
                  print();
                  return;
               }
               if (evt.getKeyText(evt.getKeyCode()).equals("Z"))
               {
                  try
                  {
                     if ( undo.canUndo() )
               ...
               if (evt.getKeyText(evt.getKeyCode()).equals("Y"))
               {
                  try
                  {
                     if ( undo.canRedo())
               ...
               if ( evt.getKeyText( evt.getKeyCode() ).equals( "L" ) )
               {
                  gui.setGUIState( BIGGUI.LOCALDEFS ) ;
                  return ;
               }
               if ( evt.getKeyText( evt.getKeyCode() ).equals( "E" ) )
               {
                  gui.executeSelected() ;
                  return ;
               }
               if ( evt.getKeyText( evt.getKeyCode() ).equals( "O" ) )
               {
                  system.BIGInterface.getInstance().
                      getBIGConfigFileParser().showConfigDialog( gui ,
                      gui.getKernelTree() ) ;
                  return ;
               }
               if ( evt.getKeyText( evt.getKeyCode() ).equals( "Q" ) )
               {
                  gui.saveAndExit() ;
                  return ;
               }
               if ( evt.getKeyText( evt.getKeyCode() ).equals( "F" ) )
               {
                  int caretPosition = 0 ;
                  String find = "" ;
                  while ( true )
                  {
                     ...
                     if ( ( find != null ) && ( !find.equals( "" ) ) )
                     {
                        try
                        {
                           String text = BIGEditor.this.getDocument().
                               getText(
                                   0 ,
                                   BIGEditor.this.getDocument().getLength() -
                                   1 ) ;
                           int position = text.toUpperCase().indexOf( find.toUpperCase() , caretPosition ) ;
                           if ( position > -1 )
                           {
                              BIGEditor.this.setCaretPosition( position ) ;
                              BIGEditor.this.setSelectionStart( position ) ;
                              BIGEditor.this.setSelectionEnd( position +
                                  find.length() ) ;
                              caretPosition = position + 1 ;
                           }
                           else
                           {
                              JOptionPane.showMessageDialog( BIGEditor.this ,
                                  "\"" + find + "\" wasn't found." ) ;
                              return ;
                           }
                        }
               ...
               if ( evt.getKeyText( evt.getKeyCode() ).equals( "G" ) )
               {
                  String line = JOptionPane.showInputDialog( BIGEditor.this ,
                      "Goto which line?" ) ;
                  try
                  {
                     int newLine = ( new Integer( line ) ).intValue() - 1 ;
                     BIGEditor.this.setCaretPosition( BIGEditor.this.
                         getLineStartOffset( newLine ) ) ;
                     BIGEditor.this.scrollTo( newLine , 0 ) ;
                     System.out.println( "There are only " + getLineCount() +
                                         " lines in this file." ) ;
                  }
            ...
            if ( evt.getKeyText( evt.getKeyCode() ).equals( "F5" ) )
               gui.getKernelTree().updateKernelTree() ;
            if ( evt.getKeyText( evt.getKeyCode() ).equals( "F6" ) )
               gui.getResultTree().updateResultTree( null ) ;
            if ( evt.getKeyText( evt.getKeyCode() ).equals( "F1" ) )
            {
      ...
      this.setInputHandler( han ) ;
      this.setCaretPosition( 0 ) ;
      java.util.Set forwardTraversalKeys = new java.util.TreeSet() ;
      ...
      java.util.Set backwardTraversalKeys = new java.util.TreeSet() ;
   ...
   public final int getMagicCaretPosition()
   {
      if (magicCaret==-1)
      {
         magicCaret=(getCaretPosition()-getLineStartOffset(getCaretLine())+5)*painter.getFontMetrics().charWidth('W');
      }
      return magicCaret ;
   }
   ...
   /**
    * Returns the text in the Editor as a <code>BIGStrings</code>-Object, splitted
    * by every line.
   ...
   /** This method tells if the text of the editor changed since
    * it's creation or the last call to <code>textSaved()</code>.
    *
   ...
   /** Resets the textChanged field of this class and sets this
    * class' text property to the current text of the JEditorPane.
    */
   ...
   /*protected void finalize()
           {
      System.err.println("remove editor@"+this.hashCode());
           }*/
   ...
   public int print(Graphics g, PageFormat pf, int pageNr) {
      ...
      int pixelsForallLines=pixelsPerLine*this.getLineCount();
      int linesPerPage=(int)((pf.getImageableHeight()/scaleFactor)/pixelsPerLine);
      int startingLineForActualPage=pageNr*(linesPerPage);
      int numberOfPagesVertical=((this.getLineCount()-1)/linesPerPage)+1;
      System.err.println("PagesVert:"+numberOfPagesVertical);
      int maxSizeHor=0;
      for (int i=0;i<this.getLineCount();i++)
         if ( maxSizeHor < this.getLineLength( i ) )
            maxSizeHor = this.getLineLength( i ) ;
      StringBuffer sb=new StringBuffer(maxSizeHor);
      for (int i=0;i<maxSizeHor;i++)
         sb.append('W');
      maxSizeHor=(int)(painter.getFontMetrics().stringWidth(sb.toString()));
      ...
      {*/
        // offSetY = offSetY-startingLineForActualPage*pixelsPerLine;//offSetY - startingLineForThisPage * ( ( int ) pf.getHeight() ) ;
